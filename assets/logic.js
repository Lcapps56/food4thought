...
$(document).ready(() => {
  favorites = () => {
    ...
    } else { $('#favbox').append('You currently have no favorites...You should fix that') }
...
$("#submitButton").on("click", function (event) {
  ...
  $.ajax({
    ...
    for (var i = 0; i < response.hits.length; i++) {
      ...
      var holder = '<button type="button" class="btn btn-primary"  data-url=' + response.hits[i].recipe.url + ' data-title=' + response.hits[i].recipe.label + ' id="likebutton">like</button>'
      var saveButton = $('<button type="button" class="btn btn-primary"  data-url=' + response.hits[i].recipe.url + ' data-title="' + response.hits[i].recipe.label + '" data-image ='+response.hits[i].recipe.image+' id="likebutton">like</button>')
      console.log(holder)
